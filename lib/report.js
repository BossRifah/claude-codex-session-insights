...
export function buildReport(threadSummaries, options = {}) {
  ...
  return {
    metadata: {
      ...
      sourceRoots: normalizeSourceRoots(options),
...
export function renderTerminalSummary(report) {
  const text = getReportText(report.metadata.language, report.metadata.source)
  ...
  <body>
    <main>
      <section class="hero">
        ...
        <p class="meta">${escapeHtml(text.generatedLabel)} ${escapeHtml(report.metadata.generatedAt)} ${escapeHtml(text.generatedFrom)} ${report.metadata.threadCount} ${escapeHtml(text.substantiveThreads)} ${escapeHtml(text.inCodexHome)} ${escapeHtml(formatSourceRoots(report.metadata))}.</p>
...
function normalizeSourceRoots(options) {
  if (Array.isArray(options.sourceRoots) && options.sourceRoots.length > 0) {
    return options.sourceRoots
      ...
      .map(root => ({ name: String(root.name), path: String(root.path) }))
  }
  return options.codexHome ? [{ name: 'Codex', path: options.codexHome }] : []
}

function formatSourceRoots(metadata) {
  const roots = Array.isArray(metadata.sourceRoots) && metadata.sourceRoots.length > 0
    ? metadata.sourceRoots
    : metadata.codexHome
      ? [{ name: 'Codex', path: metadata.codexHome }]
      : []
  return roots.map(root => formatCodexHome(root.path)).join(' + ')
}
...
function getReportText(lang, source = 'codex') {
  const sourceText = getSourceText(source, lang)
  if (lang === 'zh-CN') {
    return {
      reportTitle: sourceText.reportTitle,
      eyebrow: sourceText.eyebrow,
      generatedLabel: '生成时间',
      generatedFrom: '，基于',
...
function getSourceText(source, lang) {
  const combined = source === 'all'
  const claude = source === 'claude'
  if (lang === 'zh-CN') {
    if (combined) return { reportTitle: 'Claude + Codex 会话洞察', eyebrow: 'Claude + Codex 会话报告' }
    if (claude) return { reportTitle: 'Claude 会话洞察', eyebrow: 'Claude 会话报告' }
    return { reportTitle: 'Codex Insights', eyebrow: 'Codex 会话报告' }
  }
  if (combined) return { reportTitle: 'Claude + Codex Session Insights', eyebrow: 'Claude + Codex Usage Report' }
  if (claude) return { reportTitle: 'Claude Session Insights', eyebrow: 'Claude Usage Report' }
  return { reportTitle: 'Codex Insights', eyebrow: 'Codex Usage Report' }
}
